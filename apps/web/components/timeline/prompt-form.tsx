            ...
            <div key={id} className="block">
              ...
              {fieldType === "select" ? (
                <select
                  value={values[id] ?? ""}
                  disabled
                  className="w-full border border-border/40 bg-muted/20 px-3 py-2 text-sm text-muted-foreground outline-none"
                >
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {options.map((opt) => {
                    ...
                    return (
                      <label
                        key={val}
                        className="flex cursor-not-allowed items-center gap-2 border border-border/30 bg-muted/20 px-2.5 py-1.5 text-xs text-muted-foreground"
                      >
                        <input
                          type="checkbox"
                          checked={checked}
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full resize-y border border-border/40 bg-muted/20 px-3 py-2 text-sm text-muted-foreground outline-none placeholder:text-muted-foreground/40"
                />
              )}
            </div>
          );
        })}
        <span className="inline-flex h-9 items-center border border-border/40 bg-muted/20 px-3 text-xs font-medium text-muted-foreground">
          Input submitted
        </span>
      </div>
    );
