...
function SyncIndicator({
  ...
  const config = {
    synced: {
      ...
      className: "border-emerald-500/20 bg-emerald-500/8 text-emerald-700 dark:text-emerald-400",
      label: "CLI synced",
    },
    fallback: {
      Icon: WarningCircleIcon,
      className: "border-amber-500/20 bg-amber-500/8 text-amber-700 dark:text-amber-400",
      label: "Fallback registry",
    },
    offline: {
      Icon: XCircleIcon,
      className: "border-rose-500/20 bg-rose-500/8 text-rose-700 dark:text-rose-400",
  ...
  return (
    <div className="flex h-full items-center pr-3">
      <div className={cn("flex h-8 items-center gap-2 border px-3 text-[11px] font-medium tracking-[0.08em] uppercase", config.className)}>
              ...
              <span className="relative flex size-3.5 shrink-0 items-center justify-center">
                <section.Icon
                  className={cn(
                    ...
                    section.id === activeSection ? "opacity-0" : "group-hover:opacity-0"
                  )}
                  weight="regular"
                />
                <section.Icon
                  className={cn(
                    ...
                    section.id === activeSection ? "opacity-100" : "opacity-0 group-hover:opacity-100"
                  )}
                  weight="fill"
                />
              </span>
